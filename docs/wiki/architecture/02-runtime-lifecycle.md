...
- A shell WinUI e cliente remoto do `MicaAudio.Server`.
- O servidor standalone e o dono do runtime de `Paineis` server-capable.
- O fluxo oficial para device e `WinUI -> MicaAudio.Server -> ESP32-S3`.

## Baseline atual / transicao

- O startup nao inicializa server embedded.
- `PanelsPlaybackService` salva estado remoto de runtime; ele nao compoe nem envia frames ao device.
- Fluxos client-only, como visualizador de audio e metricas do PC, param quando o WinUI fecha.

## Startup

1. `App.OnLaunched` cria `Window` + `Frame` raiz.
2. `EnsureServicesInitialized` prepara servicos globais.
...
- `PanelsPlaybackService` mantem estado visual local da UI e escreve `PanelRuntimeStateDocument` no servidor.
- `ServerPanelRuntimeService` no `MicaAudio.Server` carrega o ultimo painel ativo e continua exibindo apos o WinUI fechar.
- O ESP32 recebe somente resultado composto pelo servidor.
...
- [App.EnsureServicesInitialized](../../../src/App.WinUI/App.xaml.cs#L1)
