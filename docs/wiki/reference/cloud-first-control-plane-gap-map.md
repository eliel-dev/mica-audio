...
| HTTP firmware latest/download | Endpoints retornam `501 NotImplemented`; catalogo oficial local (`IDeviceOfficialFirmwareCatalog`) removido por nao ter implementacao ativa. | Catalogo futuro precisa ser storage-backed e indexado por `boardModel + displayBackend + panelProfileId + firmwareProfile`, publicando somente DevKitC-1 `128x64` e Matrix Portal S3 `64x64`. |
