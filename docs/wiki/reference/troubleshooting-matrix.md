...
| Reprovisionar Wi-Fi nao abre AP | confirmar status online e logs do comando `enter_provisioning` | firmware antigo sem comando ou handler nao foi chamado | validar `ExecuteReprovisionWifiAsync` e o comando tracked `EnterProvisioning` |
| Controles de acao nao aparecem no card de resumo | conferir layout do card do dispositivo selecionado | acoes ainda estao em card separado | manter `Testar LED` e `Reprovisionar Wi-Fi` no mesmo card de nome/status |
