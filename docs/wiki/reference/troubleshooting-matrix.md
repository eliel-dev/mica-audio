...
| Device provisionado cai em `SEM WIFI` apos tirar da tomada ou apertar reset | observar o serial no boot e procurar `wifi_waiting_saved_config`, `wifi_reconnect_retry` e `wifi_reconnected` | reconnect das credenciais salvas ainda nao estabilizou ou houve regressao no fallback AP | atualizar para o hotfix de reconnect pos-reset; o portal AP automatico nao deve abrir quando `host/porta/deviceId/token` ja existem |
...
| Wizard `Novo dispositivo` nao encontra porta COM | abrir etapa 2 e marcar `Mostrar todas as portas`; conferir cabo/dado USB | porta nao mapeada por VID/PID preferencial ou cabo apenas energia | trocar cabo, atualizar lista de portas e selecionar COM manualmente |
| Wizard conclui o flash, mas o device nao entra no onboarding esperado | conferir se o app exibiu `pair code` e se o celular ve o AP `MicaAudio-Setup-xxxx` | firmware antigo, release oficial stale ou drift no fluxo AP-first | rebuild do firmware oficial, repetir `erase-all + flash` e concluir o provisioning no portal AP com `Servidor` + `pair code` |
| AP `MicaAudio-Setup-xxxx` nao aparece depois do flash | abrir `Ver mais`, usar `Copiar logs` e observar se a COM reabre a `115200`; se nao houver linhas, usar `Recapturar boot` | boot sem logs capturados, reenumeracao da COM, regressao no portal AP ou pressao de RAM interna antes do Wi-Fi | manter o wizard aberto, recapturar o boot com a serial anexada e usar os logs para validar `hello`, estado do portal e erros `wifi init`/`ESP_ERR_NO_MEM` |
