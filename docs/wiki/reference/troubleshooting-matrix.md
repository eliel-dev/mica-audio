...
| Wizard para em `Timeout aguardando handshake serial` | conferir monitor serial a `115200` e firmware em build recente | firmware antigo sem `mica.serial.v1` ou porta incorreta | atualizar firmware e repetir onboarding USB; usar fluxo manual de portal AP como fallback |
