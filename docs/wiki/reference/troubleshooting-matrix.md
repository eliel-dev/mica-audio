...
| Docker local online, mas HUB75 nao renderiza | conferir logs do container para `UDP visual LAN desabilitado` e telemetria `streamInvalidFrameCount` | helper Docker rodando com UDP visual opt-in em rede/firewall que nao entrega datagramas ao ESP | rodar `scripts/docker-server-redeploy.ps1` sem `-PreferVisualUdp`; usar `-PreferVisualUdp` apenas para teste fisico especifico |
...
| Visualizador Remote mostra `endpoint ausente` e `HTTP 404` | testar `GET /api/v1/admin/visual-endpoints` no servidor configurado | container/servidor antigo sem a rota de endpoints visuais | rodar `scripts/docker-server-redeploy.ps1` e reiniciar o WinUI em modo Remote |
