...
| Docker local online, mas HUB75 nao renderiza | conferir logs do container para `UDP visual LAN habilitado` ou `UDP visual servidor->ESP indisponivel` e telemetria `streamFramesReceived/Applied` | firewall/Docker/rota LAN bloqueando `5274/udp` do servidor para o ESP, ou device sem `LanIpAddress`/capability UDP | liberar `5274/udp`, rodar `scripts/docker-server-redeploy.ps1` e validar que o ESP reporta `visualUdpSupported`; usar `-DisableVisualUdp` so para troubleshooting por WS |
...
| AP conecta no Wi-Fi, mas o device nao aparece na UI | conferir se o server esta online, se `TrustedLanAutoRegistration=true` e se `5275/udp` esta publicado/liberado no Docker/firewall | discovery UDP bloqueado ou auto-registro LAN desabilitado | habilitar `MICA_SERVER__TRUSTEDLANAUTOREGISTRATION=true`, publicar `-p 5275:5275/udp` e usar `Servidor` no portal apenas como fallback tecnico |
| Device provisionado cai em `SEM WIFI` apos tirar da tomada ou apertar reset | capturar boot por ferramenta serial externa e procurar `boot_mem stage=before_saved_wifi_begin`, `boot_mem stage=before_hub75_init`, `wifi_waiting_saved_config`, `wifi_reconnect_retry` e `wifi_reconnected` | disputa de RAM interna/DMA entre bring-up do Wi-Fi e runtime HUB75 no boot provisionado, ou regressao na politica de reconnect | atualizar para o hotfix que sobe o STA antes do HUB75; o portal AP automatico nao deve abrir quando `host/porta/deviceId/token` ja existem |
...
| Flash manual conclui, mas o device nao entra no onboarding esperado | conferir se o celular ve o AP `MicaAudio-Setup-xxxx` no flash limpo e, depois do Wi-Fi, se o server recebe discovery | firmware antigo, release oficial stale, discovery UDP bloqueado ou drift no fluxo AP-first | baixar o firmware oficial de novo, repetir o flash externo, configurar Wi-Fi e validar discovery LAN; pair code fica apenas como fallback legado |
...
| Ferramenta serial externa nao mostra logs de boot no ESP32-S3 | conferir `ARDUINO_USB_MODE=1`, `ARDUINO_USB_CDC_ON_BOOT=1`, baud `115200` e a porta USB correta | build sem CDC-on-boot ou porta USB errada | atualizar para o perfil oficial da placa, reflashear e reabrir a ferramenta serial externa |
...
| Visualizador Remote nao envia frames ao servidor | em `Configuracoes`, conferir `servidor WS enfileirados/enviados`, reconnects e ultimo erro | admin token/URL incorretos, `/ws/v1/admin/frames` inacessivel ou container antigo | usar `Testar servidor remoto`, corrigir token/URL e rodar `scripts/docker-server-redeploy.ps1` |
| Reprovisionamento Wi-Fi precisa ser acionado | confirmar se o device esta online e se o firmware aceita `enter_provisioning` | botao saiu da UI principal por decisao operacional | usar apenas fluxo tecnico/admin ate a UX voltar a expor esse comando |
| Controles de acao nao aparecem no card de resumo | conferir layout do card do dispositivo selecionado | acoes ainda estao em card separado | manter `Testar LED` e `Remover dispositivo` no mesmo card de nome/status |
