...
4. Confirme que nao ha timeout/restricao de firewall local para HTTPS. O profile `Short` do app trabalha com budget total de aproximadamente 8 segundos.
5. Lembre que a pipeline agora aplica retry so para metodo seguro + circuit breaker; durante indisponibilidade prolongada do Open-Meteo, o app pode falhar mais rapido por alguns segundos em vez de repetir tentativas indefinidamente.
6. Revise a notificacao/log da aba `Apps`; falhas de HTTP, timeout, circuito aberto ou resposta invalida do Open-Meteo aparecem explicitamente ali.
6. Se continuar sem sugestoes, use texto manual da cidade e aplique configuracao.

## Referencias de codigo

- [CityAutocompleteService.SearchWithDiagnosticsAsync](../../../src/App.WinUI/Services/Apps/CityAutocompleteService.cs#L1) - assinatura: `Task<CitySearchResult> SearchWithDiagnosticsAsync(...)`
- [ExternalHttpClients](../../../src/App.WinUI/Infrastructure/Http/ExternalHttpClients.cs#L1) - assinatura: `IServiceCollection AddExternalHttpClients(...)`
...
- [ ] Circuit breaker nao derruba a tela; apenas antecipa feedback de indisponibilidade temporaria.
