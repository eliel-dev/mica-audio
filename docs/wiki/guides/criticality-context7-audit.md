...
| RSK-004 | Firmware versioning | versionamento automatico por build (`UTC timestamp + tag + short commit`) com fallback estatico | 2.9 | Medio | Corrigido | manter script de build gerando `firmware_version.auto.h` temporario e validando telemetria | telemetria valida em device snapshot | P1 | fallback para string estatica |
...
- [RemoteDeviceServerSecretStore.cs](../../../src/App.WinUI/Services/Devices/RemoteDeviceServerSecretStore.cs#L1) - assinatura: `internal sealed class RemoteDeviceServerSecretStore`
